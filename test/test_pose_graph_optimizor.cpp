...
void AddAllCorrectPosesIntoVisualizor(const std::vector<TVec3<Scalar>> &corr_p_wb, const std::vector<TQuat<Scalar>> &corr_q_wb) {
    // Add all poses.
...
void log(const TVec3<Scalar> &p_in, const TQuat<Scalar> &q_in, const Scalar alpha, TVec6<Scalar> &v_out) {
    v_out.head<3>() = Utility::Logarithm(q_in);
    v_out.tail<3>() = alpha * p_in;
}

void exp(const TVec6<Scalar> &v_in, const Scalar alpha, TVec3<Scalar> &p_out, TQuat<Scalar> &q_out) {
    const TVec3<Scalar> vec = v_in.head<3>();
    q_out = Utility::Exponent(vec);
    p_out = v_in.tail<3>() / alpha;
}

void J(const TVec3<Scalar> &p_A, const TQuat<Scalar> &q_A, const TVec3<Scalar> &p_A_, const TQuat<Scalar> &q_A_, const Scalar s, const Scalar alpha,
...
void DoPgoForTest(const std::vector<Pose<Scalar>> &poses, std::vector<TVec3<Scalar>> &corr_p_wb, std::vector<TQuat<Scalar>> &corr_q_wb) {
    ...
    for (uint32_t i = 0; i < N; ++i) {
        ...
        Utility::ComputeTransformInverseTransform(poses[i].p_wb, poses[i].q_wb, poses[i + 1].p_wb, poses[i + 1].q_wb, temp_p_M, temp_q_M);
    ...
    Utility::ComputeTransformInverseTransform(poses.front().p_wb, poses.front().q_wb, poses.back().p_wb, poses.back().q_wb, p_A, q_A);
    ...
    for (uint32_t i = 0; i < all_p_M.size(); ++i) {
        if (i) {
            ...
            Utility::ComputeTransformTransform(int_p_M.back(), int_q_M.back(), all_p_M[i], all_q_M[i], temp_int_p_M, temp_int_q_M);
    ...
    for (uint32_t i = 0; i < N; ++i) {
        ...
        Utility::ComputeTransformInverseTransform(p_A_, q_A_, int_p_M[i], int_q_M[i], p_A_inv_int_Mi, q_A_inv_int_Mi);
    ...
    for (uint32_t i = 0; i < poses.size(); ++i) {
        ...
        if (i) {
            ...
            Utility::ComputeTransformTransform(all_p_M[i - 1], all_q_M[i - 1], all_p_U_[i - 1], all_q_U_[i - 1], delta_p, delta_q);
            Utility::ComputeTransformTransform(corr_p_wb.back(), corr_q_wb.back(), delta_p, delta_q, corr_p, corr_q);
...
void DoPgoByPoseGraphOptimizor(const std::vector<Pose<Scalar>> &poses, std::vector<TVec3<Scalar>> &corr_p_wb, std::vector<TQuat<Scalar>> &corr_q_wb) {
